...
pub fn select_auth_fields_from_account(app_state: &AppState, target_account_id: &i32, target_auth_id: &i32) -> QueryResult<AuthFieldsFromAuthAccount> {
...
pub fn remove_auth_account_tokens(app_state: &AppState, target_account_id: &i32, target_auth_id: &i32) -> QueryResult<i32> {
    let db_connection = &mut app_state.pool.get().unwrap();
...
pub fn select_auth_account(app_state: &AppState, target_account_id: &i32, target_auth_id: &i32) -> QueryResult<AuthAccount> {
    let db_connection = &mut app_state.pool.get().unwrap();
